...
const CATEGORIES = [
    "Company", "Employees", "Operations", "Locations",
    "Payroll & Billing", "Customer SOPs",
] as const;
...
function FileDropzone({
    ...
    function validate(f: File): string | null {
        if (f.size > MAX_FILE_SIZE) return `File too large (max ${formatFileSize(MAX_FILE_SIZE)})`;
            ...
            <div className="flex items-center justify-between rounded-xl border border-manila-dark bg-cream px-4 py-3.5">
                <div className="flex items-center gap-3 min-w-0">
                    ...
                    <div className="min-w-0">
                        ...
                        <p className="text-[11px] text-steel">{formatFileSize(file.size)}</p>
                    </div>
                </div>
                <button
                    type="button"
    ...
    return (
        <div>
            <div
                ...
                className={`cursor-pointer rounded-xl border-2 border-dashed px-6 py-8 text-center transition-colors ${dragOver ? "border-rust/40 bg-rust-soft/20" : "border-manila-dark/60 bg-cream hover:border-rust/20 hover:bg-rust-soft/10"
                    }`}
            >
            ...
            {error && <p className="mt-1.5 text-[12px] font-medium text-stamp">{error}</p>}
        </div>
    );
}
...
export function AddKnowledgeModal({ open, onClose, onSave }: Props) {
    ...
    function handleSave() {
        ...
        const type =
            sourceType === "Plain Text" ? "Plain Text" :
            ...
            <div className="relative z-10 w-full max-w-xl rounded-2xl border border-manila-dark bg-paper shadow-2xl">
                ...
                <div className="flex items-center justify-between border-b border-manila-dark px-6 py-4">
                    <div>
                        ...
                        <p className="mt-0.5 text-[12px] text-steel">Add a document, PDF, or plain-text knowledge item.</p>
                    </div>
                    <button type="button" onClick={handleClose} className="rounded-lg p-1.5 text-steel-light transition-colors hover:bg-manila hover:text-steel">
                        <X size={16} />
                    </button>
                </div>
                ...
                <div className="max-h-[70vh] overflow-y-auto px-6 py-5 space-y-6">
                        ...
                        <div className="flex gap-1.5">
                            {SOURCE_TYPES.map((s) => (
                                <button
                                    key={s.value}
                                    type="button"
                                    onClick={() => { setSourceType(s.value); setFile(null); setFileError(""); }}
                    ...
                    {sourceType === "Plain Text" ? (
                        <Field label="Knowledge Content" required hint={`${plainText.length} characters`}>
                            <Textarea
                                value={plainText}
                            ...
                            />
                            {errors.plainText && <p className="mt-1 text-[12px] font-medium text-stamp">{errors.plainText}</p>}
                        </Field>
                    ) : (
                        <Field label="Upload File" required>
                            <FileDropzone accept={accept} file={file} error={fileError} onFile={handleFileSelected} />
                            {errors.file && !fileError && <p className="mt-1.5 text-[12px] font-medium text-stamp">{errors.file}</p>}
                        </Field>
                    )}
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        <div className="col-span-2">
                            <Field label="Title" required>
                                ...
                                />
                                {errors.title && <p className="mt-1 text-[12px] font-medium text-stamp">{errors.title}</p>}
                            </Field>
                        </div>
                        ...
                        <Field label="Category" required>
                            ...
                            {errors.category && <p className="mt-1 text-[12px] font-medium text-stamp">{errors.category}</p>}
                        </Field>

                        <Field label="Owner" required>
                            <Input value={owner} onChange={(e) => setOwner(e.target.value)} placeholder="e.g. HR Department" />
                            {errors.owner && <p className="mt-1 text-[12px] font-medium text-stamp">{errors.owner}</p>}
                        </Field>
                        ...
                        <Field label="Effective Date" required>
                            ...
                            {errors.effectiveDate && <p className="mt-1 text-[12px] font-medium text-stamp">{errors.effectiveDate}</p>}
                        </Field>
                    </div>

                    <Field label="Description" required hint="A concise summary of what this knowledge item contains">
                        <Textarea
                            value={description}
                            ...
                            rows={2}
                        />
                        {errors.description && <p className="mt-1 text-[12px] font-medium text-stamp">{errors.description}</p>}
                    </Field>
                </div>

                {/* Footer */}
                <div className="flex items-center justify-between border-t border-manila-dark px-6 py-4">
                    <p className="text-[12px] text-steel-light">
                        {saved ? (
                    ...
                    <div className="flex items-center gap-2.5">
                        <button type="button" onClick={handleClose} className="rounded-xl border border-manila-dark bg-cream px-4 py-2.5 text-[13px] font-semibold text-steel transition-colors hover:bg-paper-dim">
                            Cancel
                        </button>
                        <button type="button" onClick={handleSave} disabled={saved} className="rounded-xl bg-rust px-4 py-2.5 text-[13px] font-semibold text-cream transition-all duration-200 hover:bg-rust-dark hover:shadow-[0_4px_16px_-4px_var(--color-rust)/0.4] active:scale-95 disabled:cursor-not-allowed disabled:opacity-40">
