...
import type { Message, Evidence, ChatAttachment, SentAttachment } from "@/lib/intelligence";
...
const ACCEPTED_TYPES = ".pdf,.doc,.docx,.txt,.csv,.xls,.xlsx,.png,.jpg,.jpeg,.webp";
const MAX_FILES = 10;
...
export function ChatWorkspace({ messages, thinking, hasMessages, activeEvidenceId, activeEvidence, onSend, onShowEvidence }: Props) {
    ...
    const addFiles = useCallback(
        (files: FileList | File[]) => {
            ...
            function isDuplicate(file: File): boolean {
                return currentAttachments.some((a) => a.name === file.name && a.size === file.size && a.file.lastModified === file.lastModified);
            }

            function validateFile(file: File): string | null {
                if (file.size > MAX_FILE_SIZE) return `"${file.name}" exceeds 20 MB limit`;
                if (currentAttachments.length >= MAX_FILES) return "Maximum 10 files per message";
    ...
    function handleSubmit() {
        ...
        const sentAttachments: SentAttachment[] = validAttachments.map((a) => buildSentAttachment(a));
    ...
    const hasDraftContent = composerValue.trim().length > 0 || attachments.some((a) => a.status === "ready");
    ...
    const composer = (
        ...
        >
            ...
            {attachments.length > 0 && (
                <div className="flex flex-wrap gap-2.5 px-4 pt-4 pb-1 max-h-[160px] overflow-y-auto">
                    {attachments.map((att) => (
                        <div
                            key={att.id}
                        ...
                        >
                            ...
                            <div className="min-w-0">
                                <p className="truncate text-[12px] font-medium text-ink max-w-[140px]">{att.name}</p>
                                <p className={`text-[10px] ${att.status === "invalid" ? "text-stamp" : "text-steel-light"}`}>
            ...
            <div className="flex items-center justify-between px-4 pb-4">
                <div className="flex items-center gap-0.5">
                    ...
                    <button type="button" className="rounded-xl p-2.5 text-steel-light transition-colors duration-200 hover:bg-manila hover:text-steel" title="Voice input">
                        <Mic size={17} />
                    </button>
                </div>
                <button
    ...
    if (!hasMessages) {
        return (
            <div className="flex flex-1 flex-col items-center justify-center px-8 py-8">
                ...
                <ImagePreviewModal url={previewUrl} filename={previewFilename} onClose={closeImagePreview} />
            </div>
        );
    }
