...
function ConversationItem({ id, title, updatedAt, active, onClick, onRename }: { id: string; title: string; updatedAt: string; active?: boolean; onClick: () => void; onRename: (id: string, newTitle: string) => void }) {
    ...
    return (
        <div className={`group flex w-full items-start gap-3 rounded-lg px-3 py-3 text-left transition-all duration-150 ${active ? "bg-ink-soft" : "hover:bg-ink-soft"}`}>
            <button type="button" onClick={onClick} className="mt-0.5 shrink-0">
                <MessageSquare size={15} className="text-steel-light" />
            </button>
            <div className="min-w-0 flex-1" onClick={onClick} role="button" tabIndex={-1}>
                {editing ? (
                    <input
                        ref={inputRef}
                        value={editValue}
                    ...
                    <p className="truncate text-[13px] font-medium leading-snug text-cream/85">{title}</p>
                )}
                <p className="mt-0.5 text-[11px] text-steel-light/70">{updatedAt}</p>
            </div>
            {!editing && (
                <button type="button" onClick={startEditing} className="mt-0.5 shrink-0 rounded p-0.5 text-steel-light/40 opacity-0 transition-all duration-150 hover:text-steel-light group-hover:opacity-100" title="Rename">
                    <Pencil size={12} />
                </button>
            )}
        </div>
...
export function ConversationHistory({ collapsed, onToggle, conversations, activeConversationId, onSelectConversation, onNewConversation, onRenameConversation }: Props) {
    ...
    return (
        <div className="flex h-full w-[312px] flex-none flex-col border-r border-ink-line bg-ink">
            <div className="flex items-center justify-between px-4 pt-5 pb-4">
                <p className="text-[11px] font-bold uppercase tracking-[0.15em] text-cream/60">History</p>
                <button type="button" onClick={onToggle} className="rounded-lg p-1.5 text-steel-light transition-colors hover:bg-ink-soft hover:text-cream/60" title="Close sidebar">
                    <PanelLeftClose size={15} />
                </button>
            </div>

            <div className="px-3 pb-3">
                <button type="button" onClick={onNewConversation} className="flex w-full items-center gap-2.5 rounded-lg border border-ink-line bg-ink-soft px-3.5 py-2.5 text-[13px] font-medium text-cream/70 transition-all duration-150 hover:border-rust/30 hover:text-cream active:scale-[0.98]">
            ...
            <div className="px-3 pb-4">
                <div className="relative">
                    <Search size={13} className="pointer-events-none absolute left-3 top-1/2 -translate-y-1/2 text-steel-light" />
                    <input type="text" placeholder="Search..." value={search} onChange={(e) => setSearch(e.target.value)} className="w-full rounded-lg border border-ink-line bg-ink-soft py-2 pl-9 pr-3 text-[13px] text-cream/70 placeholder:text-steel-light outline-none transition-all duration-200 focus:border-rust/30" />
                </div>
            </div>
            ...
            <div className="flex-1 overflow-y-auto px-2 pb-5">
                <div className="space-y-5">
                    {today.length > 0 && (
                        <section>
                            <p className="mb-1.5 px-3 text-[10px] font-bold uppercase tracking-[0.15em] text-cream/35">Today</p>
                            {today.map((c) => {
                                const now = Date.now();
                    ...
                    {yesterday.length > 0 && (
                        <section>
                            <p className="mb-1.5 px-3 text-[10px] font-bold uppercase tracking-[0.15em] text-cream/35">Yesterday</p>
                            {yesterday.map((c) => {
                                const now = Date.now();
                                const age = now - c.updatedAt;
                                return (
                    ...
                    {previous.length > 0 && (
                        <section>
                            <p className="mb-1.5 px-3 text-[10px] font-bold uppercase tracking-[0.15em] text-cream/35">Previous</p>
                            {previous.map((c) => {
                                const now = Date.now();
                                const age = now - c.updatedAt;
                                return (
