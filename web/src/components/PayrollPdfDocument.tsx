...
const styles = StyleSheet.create({
    ...
    header: { marginBottom: 24, borderBottomWidth: 1, borderBottomColor: "#e2e8f0", borderBottomStyle: "solid", paddingBottom: 16 },
    ...
    tableHeader: { flexDirection: "row", backgroundColor: "#f1f5f9", borderBottomWidth: 1, borderBottomColor: "#e2e8f0", borderBottomStyle: "solid" },
    tableHeaderCell: { padding: "6 8", fontSize: 7, fontWeight: 600, color: "#64748b" },
    ...
    footer: { marginTop: 20, borderTopWidth: 1, borderTopColor: "#e2e8f0", borderTopStyle: "solid", paddingTop: 12, flexDirection: "row", justifyContent: "space-between" },
...
export function PayrollPdfDocument({ title, subtitle, rows, locationName }: Props) {
            ...
            <Page size="LETTER" style={styles.page}>
                ...
                <View style={styles.table}>
                    ...
                    <View style={styles.tableHeader}>
                        ...
                        <Text style={[styles.tableHeaderCell, styles.colProduction]}>Prod</Text>
                    ...
                    {rows.map((r) => (
                        <View style={styles.tableRow} key={r.employee.id}>
                            <Text style={[styles.tableCell, styles.colEmployee]}>{r.employee.name}</Text>
                            <Text style={[styles.tableCell, styles.colCategory]}>{r.employee.category ?? "\u2014"}</Text>
                            <Text style={[styles.tableCell, styles.colLocation]}>{locationName(r.employee.locationId)}</Text>
                            <Text style={[styles.tableCellRight, styles.colLoads]}>{String(r.entries.length)}</Text>
                            <Text style={[styles.tableCellRight, styles.colHours]}>{fmtHours(r.totalHours)}</Text>
                            <Text style={[styles.tableCellRight, styles.colBase]}>{fmtMoney(r.regularPay)}</Text>
                            <Text style={[styles.tableCellRight, styles.colOvertime]}>{fmtMoney(r.overtimePay)}</Text>
                            <Text style={[styles.tableCellRight, styles.colProduction]}>{fmtMoney(r.productionPay)}</Text>
                            <Text style={[styles.tableCellRight, styles.colTotal, styles.money]}>{fmtMoney(r.totalPay)}</Text>
                        </View>
                    ))}
                </View>
