...
export function getPayPeriods(loads: Load[]): PayPeriod[] {
    ...
    while (cursor <= last) {
        ...
        periods.push({ id: start, label: `${fmt(cursor)} \u2013 ${fmt(endDate)}`, startDate: start, endDate: end });
...
export function filterLoadsByPeriod(loads: Load[], period: PayPeriod): Load[] {
    return loads.filter((l) => l.date >= period.startDate && l.date <= period.endDate);
}

const OVERTIME_THRESHOLD = 40;

export function calcOvertimeHours(totalHours: number) {
    if (totalHours <= OVERTIME_THRESHOLD) return { regular: totalHours, overtime: 0 };
    return { regular: OVERTIME_THRESHOLD, overtime: Math.round((totalHours - OVERTIME_THRESHOLD) * 100) / 100 };
}
...
export type PayrollPaymentMethod = "cash" | "check" | "direct_deposit" | "bank_transfer" | "other";
...
export function getEmployeePayroll(
    ...
    for (const load of loads) {
        ...
        entries.push({
            loadId: load.id, ticketNumber: load.ticketNumber, date: load.date,
            customerName: getCustomerName(load.customerId),
            productTypeName: getProductTypeName(load.productTypeId),
            clockIn: a.clockIn, clockOut: a.clockOut, hours,
    ...
    const overtimePay = Math.round(overtime * employee.hourlyRate * 1.5 * 100) / 100;
    ...
    return {
        ...
        totalPay: Math.round((regularPay + overtimePay + totalProductionPay) * 100) / 100,
...
export function getAllPayroll(
    employees: Employee[], loads: Load[],
    getCustomerName: (id: string) => string,
    getProductTypeName: (id: string) => string,
): EmployeePayroll[] {
    return employees
        .filter((e) => e.status === "active")
        .map((e) => getEmployeePayroll(e, loads, getCustomerName, getProductTypeName))
